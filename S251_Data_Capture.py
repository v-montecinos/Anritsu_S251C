...
def _savgol_safe(y, window_length=51, polyorder=3):
    """Savitzky–Golay with window capped to a valid odd length <= len(y)."""
    n = len(y)
    if n <= polyorder:
        return y
    w = min(window_length, n if n % 2 else n - 1)
    if w <= polyorder:
        return y
    if w % 2 == 0:
        w -= 1
...
def plot_data(
    ...
    y = fn(gamma)
    if smooth:
        y = np.round(_savgol_safe(y, 51, poly), 3)
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--smooth",
        action="store_true",
        help="Enable Savitzky-Golay smoothing for the trace and CSV export",
    )
    p.add_argument(
        "--csv",
        metavar="FILE",
